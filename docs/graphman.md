...
#### `--ttl-days <TTL_DAYS>`
The `--ttl-days` option is used to remove stale contracts based on the `call_meta` table. The value of `TTL_DAYS` is the number of days after which a contract is considered stale if it has not been accessed. For example, if `TTL_DAYS` is set to 30, any calls to a contract that has not been accessed in the last 30 days will be removed from the call cache.
...
Remove stale contracts from the call cache that have not been accessed in the last 30 days:

    graphman --config config.toml chain call-cache ethereum remove --ttl-days 30
